...
### Adding a habit doesn't lock the rest of the habit list

**Decision:** while `AddHabitForm`'s submission is pending, only the add-habit form's own controls (input, submit, cancel) and the header "Add habit" trigger button are disabled. Existing habit cards - their checkboxes, edit icons, and delete icons - stay fully interactive; nothing about them is blocked while a new habit is being added. `docs/design/add-habit-form-states.html`'s pending-state mockup was updated to match - the habit cards render normally there, with no dimming, and only the form and header button appear disabled.

**Why:** the same independence principle as the toggle decision above applies across features, not just within one. Adding a new habit and acting on a different, existing habit touch unrelated rows, with no shared server-side state between them - there's no correctness reason to block one while the other is in flight. Blocking them anyway would add friction for no benefit, and would require exactly the kind of shared "is anything pending" coordination the toggle design deliberately avoided by keeping each mutation's pending state local and independent (see above - no manual `pendingHabitIds` bookkeeping needed, and the same reasoning extends here). What's genuinely worth disabling is narrower: the add-habit form's own controls (prevents literally double-submitting the same create request) and the header trigger button specifically (clicking it again mid-submission would prematurely close the form - a real correctness issue, not just UX polish) - not anything unrelated to the add-habit flow itself.
